...
function processPOIData(data) {
    if (data.ok && data.results.length > 0) {
        ...
        $j('#POI input:checkbox').each(function () {
            if ($j(this).attr('value') == POI.current_label) {
                $j(this).attr('checked', false).attr('disabled', true);
            }
        });
        ...
        if (POI.sub_poi) {
            $j("#POI_header, #POI_NoBoundMarkers").hide();
            $j("#POI_NoResults").show();
        }
        POI.setLoader(false);
    }
}
...
function POILocator() {
    ...
    this.sub_poi = false;
    this.image_link = '/z/webagency/slagence_X_V3/images/poi/poi_';
    this.latitude = '';
    this.longitude = '';
    this.current_title = '';
    this.current_label = '';
...
POILocator.prototype.getPOIFromMaster = function(masterBox) {
    ...
    children.each(function () {
        ...
        if (masterBox.is(':checked')) {
            ...
            POIPointer.categoryArray.push($j(this));
        } else {
            $j(this).attr('checked', '');

            // reset the category id for removal
            for (var i = 0; i < POIPointer.poi_ref_chart.length; i++) {
                if (POIPointer.poi_ref_chart[i].label == $j(this).attr('value')) {
...
POILocator.prototype.removePOI = function() {
    ...
    $j("#POI_NoResults, #POI_NoBoundMarkers").hide();
    $j("#POI_header").show();
    this.setLoader(false);
}
...
POILocator.prototype.processData = function(JSONObject) {
    ...
    if (JSONObject) {
        $j.each(JSONObject.results, function (index, data) {
            if (! data.place.closed) {
                $j("#POI_NoResults").hide();
                $j("#POI_header").show();
        ...
        if (! POIPointer.boundCheck) {
            $j("#POI_header").hide();
            $j("#POI_NoBoundMarkers").show();
        } else {
            $j("#POI_NoBoundMarkers").hide();
            $j("#POI_header").show();
        }
    }
