function setGeoMap(isRealEstate, cityscanData, colorLayer, urwVille, urwDepartement, urwRegion, language, cityscan_url) {
	...
	if (cityscanData != null) {
		...
		var baseUrl = (cityscan_url == null || cityscan_url == "") ? "https://www.cityscan.fr/widget?clientKey=" : cityscan_url + "/widget?clientKey=";
		
		iframeCityscan.src = baseUrl + cityscanData.ck + "&addressId=" + cityscanData.idcs + (cityscanData.nl=='True'?'&noLoc':'') + "&language=" + (language=="fr"?"fr-FR":"en-US") + (cityscanData.rd!=""?("&radius="+cityscanData.rd):"");
...
function getCityscan(idAnnonce, idTypeTransaction, colorLayer, urwVille, urwDepartement, urwRegion, language) {
	if (idAnnonce != '' || idTypeTransaction != '') {
			...
			success: function(data) {
				if (data.toString().length > 0) {
					...
					if (json.success) {
						...
						if (json.data.idcs != '') {
							setGeoMap(true, json.data, colorLayer, urwVille, urwDepartement, urwRegion, language, json.cityscan_url);
						}
						else {
							setGeoMap(true, null, colorLayer, urwVille, urwDepartement, urwRegion, language, json.cityscan_url);
						}
					}
				}
			}
		...
		setGeoMap(false, null, colorLayer, urwVille, urwDepartement, urwRegion, language, null);
	}
}
