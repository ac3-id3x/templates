...
function countSelection(nbAnnonces) {
	...
	// Dans le cas où le chariot apparait dans la barre des médias sociaux
	if(typeof fixCartProperties !== "undefined" && fixCartProperties) {
		for(var i = 0; i < fixCartProperties.selectors.length; i++) {
			for(var j = 0; j < fixCartProperties.selectors[i].attributes.length; j++) {
				var value = $(fixCartProperties.selectors[i].element).attr(fixCartProperties.selectors[i].attributes[j]);
				
				if(value) {
					var valueBeforeSeparator = value.substring(0, value.indexOf(fixCartProperties.separator) + 1);
					
					var newValue = valueBeforeSeparator + ' ' + nbAnnonces + ' ';
						newValue += (nbAnnonces > 1) ? fixCartProperties.plural : fixCartProperties.singular;
					
					$(fixCartProperties.selectors[i].element).attr(fixCartProperties.selectors[i].attributes[j], newValue);
				}
			}
		}
	}
			...
			$.ajax({
				...
				success: function(data) {
					...
					if($this.children().find('i').length > 0){
						$this.children().children().find('.checkmark').addClass('display-none').children('i').css('color','');
