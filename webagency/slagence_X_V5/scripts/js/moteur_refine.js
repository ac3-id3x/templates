...
//Recup de l'html d'un element moteur pour reinjection
var elmntmoteur = $('#content-moteur .conteneur-checkbox').first().clone();
var elmntmoteurligne = elmntmoteur.find('.ligne').first().clone();
//Definition De Variables
var noqry="";
var eachAnnonce ='';
//RECUP DE COOKIE SI EXISTE
$(document).ready(function() {
	...
	$('#recherche-moteur').on('click','.ligne',function(){
		if(!$(this).hasClass('refineIdtt')){
			...
			$('.refineIdtt').find('.json-check').removeClass('display-show');
			// FIX NEW CLICK IDTT
			$(this).addClass(btnaction+" typo-white").removeClass(btnneutre);
			$(this).find('.json-check').addClass('display-show');
			// LOAD VILLES
	...
	//REMISE A ZERO
	$('.container').on('click', '.bouton-raz', function (e){
		e.preventDefault();
		if (noqry = 'undefined' ){
			var noqry = 'idtt='+idtt ;
		}else{
	...
	$('#recherche-moteur').on('click', '.bouton-rechercher-refine, .bouton-rechercher-moteur', function (e){
		e.preventDefault();
		$(".titre-section-refine").next().removeClass('display-show').addClass('display-none');
		$(".titre-section-refine").find('.minus').addClass('display-none');
		$(".titre-section-refine").find('.plus').removeClass('display-none');
		$('.hidden-annlistepg').remove();
		loadInfos();
	});
	...
	$('#recherche-moteur').on('click', '.titre-section-refine', function (){
		...
		if (liste.hasClass('display-none')){
			if(liste.hasClass('display-show')) {
				...
				//$(this).parent('div').children('.titre-section-refine').children('div').children('i').removeClass('general typoicon-minus');
				//$(this).parent('div').children('.titre-section-refine').children('div').children('i').addClass('general typoicon-plus');
			} else {
				liste.addClass('display-show');
				$(this).find('.minus').removeClass('display-none');
				$(this).find('.plus').addClass('display-none');
				//$(this).parent('div').children('.titre-section-refine').children('div').children('i').removeClass('general typoicon-plus');
				//$(this).parent('div').children('.titre-section-refine').children('div').children('i').addClass('general typoicon-minus');
			}
		}
	});
});
...
function champshidden($this){
	var thistitre = $this.parent('div').parent('div').parent('div').parent('div').parent('div').children('.titre-section-refine').attr('data-name');
	if(thistitre==undefined){
		var thistitre = $this.attr('data-name');
		if($('.hidden-'+thistitre).val()==undefined){
			$('#Mini_Moteur').prepend('<input type="hidden" value="" name="'+thistitre+'" class="hidden-'+thistitre+' auto-create-hidden" />');
		}
...
function loadville(qry,ci){
	$.ajax({
		...
		cache: false,
		data : qry,
		url: '/moteur,refine,incl_villes_refine.htm',
					...
					if($('#moteurRefine').val()==1){
						if(attr[0]=='ci'){
							...
							if($('.hidden-'+attr[0]).val()!=undefined){
								...
								if (attr[0]!='pxmax'&&attr[0]!='pxmin'&&attr[0]!='surfacemax'&&attr[0]!='surfacemin'&&attr[0]!='surf_terrainmin'&&attr[0]!='surf_terrainmax'&&attr[0]!='pxbtw'){
									$('#Mini_Moteur').prepend('<input type="hidden" value="" name="'+attr[0]+'" class="hidden-'+attr[0]+' auto-create-hidden" />');
									$('.hidden-'+attr[0]).val(fullattr);
								}else{
...
function usePagination(maxpage){
	$('#recherche-listing').on('click', '.pagination a,.pagination div', function(e){
		...
		if($('.hidden-annlistepg').length<1){
			$('#Mini_Moteur').prepend('<input type="hidden" value="" name="ANNLISTEpg" class="hidden-annlistepg auto-create-hidden" />');
		}
		if(num == '&lt;&lt;'){
			...
			if(data.idtt != ""){
				...
				$('.refineIdtt').each(function(){
					if($(this).attr('data-idtt')==data.idtt){
						$(this).addClass(btnaction+" typo-white").removeClass(btnneutre);	
						$(this).find('.json-check').removeClass('display-none');		
					}else{
						$(this).addClass(btnneutre).removeClass(btnaction+" typo-white");
						$(this).find('.json-check').addClass('display-none');
					}
				});
