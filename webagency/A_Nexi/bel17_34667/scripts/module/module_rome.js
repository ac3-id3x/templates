$(document).ready(function() {
	...
	$('div[id*=moduleRome]').find('ul').each(function() {
		...
		$(this).children('.annonceFlipping').hover(
			function() {
				$(this).children('.contentAnnonceFlipping').slideDown(500);
			},
			function() {
				$(this).children('.contentAnnonceFlipping').slideUp(500);
			}
		);	
			if($countLi > 4) {
...
var moduleDiv = 	$('.rotateRome');
		...
		var objModule = {	
			...
			decalage4: ($('#masqueModuleRome4').children('ul').find('li').outerWidth(true)) * 4,
			...
			callDelayed:function(num) {
				if(num == 1) {
					...
					timerInterval4 = setInterval(function() { objModule.callSlide(); }, defileRomeSpeed);
				} else {
	...
	var startSlide = function() {
		...
		var $container4 = $("."+this.idDiv).children('#masqueModuleRome4').children('ul');
			...
			if($calcul4 > (-1 * $max4)) {
				$container4.animate({marginLeft:$calcul4+'px'},500);
			} else {
				$container4.animate({marginLeft:0},500);
			}
	};
