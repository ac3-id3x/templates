function setGeoMap(isRealEstate, cityscanData, colorLayer, urwVille, urwDepartement, urwRegion) {
	...
	if (cityscanData != null) {
		...
		iframeCityscan.src = "https://www.cityscan.fr/widget?clientKey=" + cityscanData.ck + "&addressId=" + cityscanData.idcs + (cityscanData.nl!=''?'&noLoc':'');// + "&height=500&width=700";
...
function getCityscan(idAnnonce, idTypeTransaction, colorLayer, urwVille, urwDepartement, urwRegion) {
	if (idAnnonce != '' || idTypeTransaction != '') {
			...
			success: function(data) {
				if (data.toString().length > 0) {
					...
					if (json.success) {
						...
						if (json.data.idcs != '') {
							setGeoMap(true, json.data, colorLayer, urwVille, urwDepartement, urwRegion);
						}
						else {
							setGeoMap(true, null, colorLayer, urwVille, urwDepartement, urwRegion);
						}
					}
				}
			}
		...
		setGeoMap(false, null, colorLayer, urwVille, urwDepartement, urwRegion);
	}
}
