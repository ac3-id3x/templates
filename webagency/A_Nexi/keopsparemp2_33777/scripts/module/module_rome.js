...
var moduleDiv = 	$('.rotateRome');
	...
	var startSlide = function() {
		...
		var $container3 = $("."+this.idDiv).children('#masqueModuleRome3').children('ul');
			...
			if($calcul > (-1 * $max)) {
				$container.animate({marginLeft:$calcul+'px'},2000);
			} else {
				$container.animate({marginLeft:0},2000);
			}
			if($calcul2 > (-1 * $max2)) {
				$container2.animate({marginLeft:$calcul2+'px'},2000);
			} else {
				$container2.animate({marginLeft:0},2000);
			}
			if($calcul3 > (-1 * $max3)) {
				$container3.animate({marginLeft:$calcul3+'px'},2000);
			} else {
				$container3.animate({marginLeft:0},2000);
			}
	};
