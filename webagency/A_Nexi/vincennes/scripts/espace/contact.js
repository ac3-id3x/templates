		...
		success: function(data) {
			...
			if (reloadMap) {
				...
				if (document.getElementById(elementId)) {
					...
					document.getElementById(elementId).innerHTML = '<div id="' + nodeId + '" style="width: 100%; height: 100%; -webkit-filter:grayscale(100%); filter:grayscale(100%);"></div>';
					...
					id3xContent.addMap({
						container: {
							id: nodeId,
							cssStyle: {
								addStyle: '-webkit-filter:grayscale(100%); filter:grayscale(100%);' // fix for : map size + grey filter
							}
						},
						mapData: [{
