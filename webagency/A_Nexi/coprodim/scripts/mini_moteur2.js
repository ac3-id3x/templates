/*defini les onglets carte ou liste */
	function ChoixTypeRech(id){
      ...
      if(id == 'l'){
        		pageResultat.action = '/recherche,$$PAGE:WAG_STYLE_RECHERCHE$$.htm';		
     }else if(id == 'c'){        		
        		pageResultat.action = '/map,recherche.htm';        		
     }
 	}


$$REM$$/* 06.07.2007 
				il semble que ce script était buggé, pourquoi on sait pas
				mais on l'a remplacé par celui ci-dessus
				signé Lisbonne et Christophe
*/$$END$$
/*defini les onglets carte ou liste */
//	function ChoixTypeRech(id){
...
function BudgetTranche(idtt){
	...
	if(divlistebudget && divlisteloyer){
		var selectbudget = divlistebudget.getElementsByTagName("select");
		var selectloyer = divlisteloyer.getElementsByTagName("select");
		if(idtt){
		divbudgettranche.style.display = "inline";
		if(idtt == "1"){
		 divlisteloyer.style.display = "inline";
		 divlistebudget.style.display = "none";
		 if(selectbudget.length != 0){selectbudget.pxbtw.selectedIndex = 0;}
		}else{
			 divlistebudget.style.display = "inline";
		 divlisteloyer.style.display = "none";
		if(selectloyer.length != 0){ selectloyer.pxbtw.selectedIndex = 0;}
		}
	}else{
...
function Envoi(){
			...
			if ( document.getElementById("MiniSelectRegion").value==""){
	...
	var tableau = boite.getElementsByTagName("input");
		...
		if(divminivilles && divselminivilles){
				...
				//alert(selectvilles.selectedIndex);
				if(selectvilles.ci.selectedIndex != 0){
					if(divminidep){
						var selectdep = divminidep.getElementsByTagName("select");
						selectdep.cp.selectedIndex = '';
					}
					if(divminiregion){
						var selectregion = divminiregion.getElementsByTagName("select");
						selectregion.cp.selectedIndex = '';
					}
				}else{
					if(divminidep){
						var selectdep = divminidep.getElementsByTagName("select");
						if(selectdep.cp.selectedIndex != ''){
							if(divminiregion){
							var selectregion = divminiregion.getElementsByTagName("select");
								selectregion.cp.selectedIndex = '';
							}
		...
		if(budg){
			...
			var budgmin = document.getElementById("pxhidden");
			if(ventediv){
