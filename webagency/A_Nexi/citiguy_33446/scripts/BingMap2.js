...
{  
  ...
  // Event handler
	...
	bMap.AttachEvent("dblclick",Map_OnDblClick);
...
{	
	...
	gMap.LoadMap(new VELatLong(arrayOfAgencies[0].latitude,arrayOfAgencies[0].longitude),14,'r',true);
	gMap.SetScaleBarDistanceUnit(VEDistanceUnit.Kilometers);
	gMap.SetMapView(VELatLongArray);	
	
	gMap.AttachEvent("onmousewheel",Map_OnMouseWheel);
	gMap.AttachEvent("dblclick",Map_OnDblClick);
...
{			
	...
	//var html = '<div id="listeAg">'+oAgency.idPublication+'</div>';
	
	//pinShape.SetDescription(html);	
  gMap.AddShape(pinShape);
  document.getElementById(pinShape.GetPrimitive().iid).onclick = function(){
  	$("#wrapdefile").hide();
  	$('.blocAgenceCarte').hide();
    $('#adrCarte_'+oAgency.idPublication).fadeIn("500");  	
	}
...
{	
	...
	if(oCity.nombre == 1) {
		html = html + '<div class="descGeolocUrl">'+oCity.nombre+' '+annSingulier+'</div>';
	} else {
		html = html + '<div class="descGeolocUrl">'+oCity.nombre+' '+annPluriel+'</div>';
	}
	html = html +'<div class="descGeolocDetail"><a href="'+urlRecherche+'?'+queryRecherche+'&ci='+oCity.ci+'&lang='+langue+'">'+ linkPlusDetails +'</a></div>';
	html = html +'</div>';
	pinShape.SetDescription(html);	
  	gMap.AddShape(pinShape);
}
