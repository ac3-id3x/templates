...
function GetAnnonceMap(latitude, longitude, DashboardSize)
{
	...
	aMap.LoadMap(latlong,10,'r',false);	
...
{		
	...
	{		
		...
		{				
			...
			if(pin.infos.localisation=="ann")
				GetLayoutContent('/carto,ajaxmap,getAnnonceLoc.htm?idpublication='+ pin.infos.idpublication +'&photo=1&tri=a_px&'+ localisation +'&'+ queryPreliste());
			else
				GetLayoutContent('/carto,ajaxmap,getAnnonces.htm?photo=1&tri=a_px&'+ localisation +'&'+ queryPreliste());
		}else{
			GetLayoutContent('/carto,ajaxmap,getAnnonces.htm?photo=1&tri=a_px&'+ localisation +'&'+ queryPreliste());
		}
