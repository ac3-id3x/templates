...
$(document).ready(function() {
	...
	$('.btn-estimation-envoyer').on('click',function(e) {
		...
		if($('#email_from').val().length > 0) {
			if(regEmail.test($('#email_from').val())) {
				$('.message-erreur-step-3-email').hide();
				$('#email_from').parent().addClass('success').removeClass('error');
				validation.email = $('#email_from').val();
			} else {
...
/* FUNCTION */
function sliderEtape(slider,bouton) {
	slider.show(100);
}
