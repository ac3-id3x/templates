...
$(document).ready(function(){
	...
	$('.fermerMoteur').toggle(function(){
		$('#mi-moteur').slideUp("slow");
		$(this).text('Ouvrir le moteur');
	},function(){
		$('#mi-moteur').slideDown("slow");
		$(this).text('Fermer le moteur');
	});	
});
/* POP */
...
function servicesBloc(divid) {
	...
	$gauche.click(function() {
		...
		var $prevBloc = $actifBloc.prev();
		//alert($nextBloc.length);
		if($prevBloc.length == 0) {
