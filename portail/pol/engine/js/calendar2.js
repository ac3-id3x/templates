...
//	email  : pinotoy@yahoo.com
	...
	var language = 'fr';	// Default Language: en - english ; es - spanish; de - german
	...
	var imgDir = '/z/portail/pol/engine/images/calendar/';		// directory for images ... e.g. var imgDir="/img/"
	var dayName = '';
	...
	var selectMonthMessage = {
		...
		fr : 'Cliquer pour selectionner un mois'
	};
	var selectYearMessage = {
		en : 'Click to select a year.',
		es : 'Presione para seleccionar un año',
		de : 'Klicken um Jahr auszuwählen',
		fr : 'Cliquer pour selectionner une année'
	};
	...
	var	monthName2 = {
		...
		fr : new Array('JAN','FEV','MAR','AVR','MAI','JUI','JUL','AOU','SEP','OCT','NOV','DEC')
	};
	...
	if (startAt==0) {
		...
		dayName2 = {
			...
			es : new Array('Lun','Mar','Mie','Jue','Vie','Sab','Dom'),
			de : new Array('Mo','Di','Mi','Do','Fr','Sa','So'),
			fr : new Array('Lundi','Mardi','Mercredi','Jeudi','Vendredi','Samedi','Dimanche')
		};
	}
	...
	*/
	...
	if (dom) {
		...
		document.write ('<div onclick="bShow=true" id="calendar" style="z-index:+999;position:absolute;visibility:hidden;"><table width="'+((showWeekNumber==1)?250:220)+'" style="font-family:Arial;font-size:11px;border: 1px solid #A0A0A0; z-index:+999; bgcolor="#ffffff"><tr><td style="padding:0px"><table width="100%" cellpadding="0" cellspacing="0"><tr bgcolor="#f0f3f5"><td width="18">&nbsp;</td><td style="font-family:Arial;font-size:11px;width:100%;"><font color="#2e4161' + '' /*C9D3E9*/ +'"><b><div id="caption"></div></b></font></td><td width="18"><a href="javascript:hideCalendar()"><img src="'+imgDir+'pcalclose.gif" width="15" height="13" border="0" /></a></td></tr></table></td></tr><tr><td style="padding:5px" bgcolor="#ffffff"><span id="content"></span></td></tr>');
	...
	function init_Cal() {
		if (!ns4)
		{
			...
			if (showToday == 1) {
				document.getElementById('lblToday').innerHTML =	'<font color="#000000">' + todayString[language] + ' <a onmousemove="window.status=\''+gotoString[language]+'\'" onmouseout="window.status=\'\'" title="'+gotoString[language]+'" style="'+styleAnchor+'" href="javascript:monthSelected=monthNow;yearSelected=yearNow;constructCalendar();">'+dayName2[language][(today.getDay()-startAt==-1)?6:(today.getDay()-startAt)]+', ' + dateNow + ' ' + monthName[language][monthNow] + ' ' + yearNow + '</a></font>';
			}

			sHTML1 = '<span id="spanLeft" style="border:1px solid #00496e;cursor:pointer" onmouseover="swapImage(\'changeLeft\',\'pcalleft2.gif\');this.style.borderColor=\'#8af\';window.status=\''+scrollLeftMessage[language]+'\'" onclick="decMonth()" onmouseout="clearInterval(intervalID1);swapImage(\'changeLeft\',\'pcalleft1.gif\');this.style.borderColor=\'#00496e\';window.status=\'\'" onmousedown="clearTimeout(timeoutID1);timeoutID1=setTimeout(\'StartDecMonth()\',500)" onmouseup="clearTimeout(timeoutID1);clearInterval(intervalID1)">&nbsp<img id="changeLeft" src="'+imgDir+'pcalleft1.gif" width="10" height="11" border="0">&nbsp</span>&nbsp;';
			sHTML1 += '<span id="spanRight" style="border:1px solid #00496e;cursor:pointer" onmouseover="swapImage(\'changeRight\',\'pcalright2.gif\');this.style.borderColor=\'#8af\';window.status=\''+scrollRightMessage[language]+'\'" onmouseout="clearInterval(intervalID1);swapImage(\'changeRight\',\'pcalright1.gif\');this.style.borderColor=\'#00496e\';window.status=\'\'" onclick="incMonth()" onmousedown="clearTimeout(timeoutID1);timeoutID1=setTimeout(\'StartIncMonth()\',500)" onmouseup="clearTimeout(timeoutID1);clearInterval(intervalID1)">&nbsp<img id="changeRight" src="'+imgDir+'pcalright1.gif" width="10" height="11" border="0">&nbsp</span>&nbsp;';
			sHTML1 += '<span id="spanMonth" style="border:1px solid #00496e;cursor:pointer" onmouseover="swapImage(\'changeMonth\',\'pcaldrop2.gif\');this.style.borderColor=\'#8af\';window.status=\''+selectMonthMessage[language]+'\'" onmouseout="swapImage(\'changeMonth\',\'pcaldrop1.gif\');this.style.borderColor=\'#00496e\';window.status=\'\'" onclick="popUpMonth()"></span>&nbsp;';
			sHTML1 += '<span id="spanYear" style="border:1px solid #00496e;cursor:pointer" onmouseover="swapImage(\'changeYear\',\'pcaldrop2.gif\');this.style.borderColor=\'#8af\';window.status=\''+selectYearMessage[language]+'\'" onmouseout="swapImage(\'changeYear\',\'pcaldrop1.gif\');this.style.borderColor=\'#00496e\';window.status=\'\'" onclick="popUpYear()"></span>&nbsp;';
	...
	function closeCalendar() {
		...
		document.getElementById(idDtJ).value = dateSelected;
		document.getElementById(idDtM).value = monthSelected + 1;
		document.getElementById(idDtA).value = yearSelected;
	}
	...
	function constructMonth() {
		...
		if (!monthConstructed) {
			...
			for (i=0; i<12; i++) {
				...
				sHTML += '<tr><td id="m' + i + '" onmouseover="this.style.backgroundColor=\'#909090\'" onmouseout="this.style.backgroundColor=\'\'" style="cursor:pointer" onclick="monthConstructed=false;monthSelected=' + i + ';constructCalendar();popDownMonth();event.cancelBubble=true"><font color="#003e62">&nbsp;' + sName + '&nbsp;</font></td></tr>';
			}
	...
	function constructYear() {
		...
		sHTML = '';
		if (!yearConstructed) {
			sHTML = '<tr><td align="center" onmouseover="this.style.backgroundColor=\'#909090\'" onmouseout="clearInterval(intervalID1);this.style.backgroundColor=\'\'" style="cursor:pointer" onmousedown="clearInterval(intervalID1);intervalID1=setInterval(\'decYear()\',30)" onmouseup="clearInterval(intervalID1)"><font color="#003e62">-</font></td></tr>';
			...
			for ( i = (yearSelected-3); i <= (yearSelected+3); i++ ) {
				...
				sHTML += '<tr><td id="y' + j + '" onmouseover="this.style.backgroundColor=\'#909090\'" onmouseout="this.style.backgroundColor=\'\'" style="cursor:pointer" onclick="selectYear('+j+');event.cancelBubble=true"><font color="#003e62">&nbsp;' + sName + '&nbsp;</font></td></tr>';
				j++;
			}

			sHTML += '<tr><td align="center" onmouseover="this.style.backgroundColor=\'#909090\'" onmouseout="clearInterval(intervalID2);this.style.backgroundColor=\'\'" style="cursor:pointer" onmousedown="clearInterval(intervalID2);intervalID2=setInterval(\'incYear()\',30)" onmouseup="clearInterval(intervalID2)"><font color="#003e62">+</font></td></tr>';
	...
	function constructCalendar () {
		...
		for (i = 0; i<7; i++) {
			sHTML += '<td width="27" align="right"><b><font color="#003e62">' + dayName[language][i] + '</font></b></td>';
		}
		...
		for	( datePointer=1; datePointer <= numDaysInMonth; datePointer++ ) {
			...
			if ((datePointer == dateNow) && (monthSelected == monthNow) && (yearSelected == yearNow)) {	///// today
				...
				if (isPast==1)
					...
					sHTML += "<a "+dateMessage+" title=\"" + sHint + "\" style='"+sStyle+"' "+selDayAction+">&nbsp;<font color=#003e62>" + datePointer + "</font>&nbsp;</a>";
			}
	...
	function showCalendar(ctl, ctl2, format, lang, past, fx, fy, idj, idm, ida) {
		idDtJ = idj;
		idDtM = idm;
		idDtA = ida;
