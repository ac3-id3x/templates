...
function GetCommerciaux(monurl, mydiv) {
	var mesparametres = 'idressource=' + $('#responsable').val();
...
function GetComToAssoc(elt) {
	...
	if ($('#' + elt)) {
		var list_id = $('#' + elt).val();  // On récupére la liste des ids de la liste box elt
		if (list_id === "") {
            alert("Veuillez sélectionner au moins un commercial.");
        } else {
			$("#idcommerciaux").val(list_id);
			if ($("#idcommerciaux").val() !== "") {
                frmressource.submit();
            }
		}
	} else {
...
function validate(sender, myarray, err_hd) {
	...
	if (elt_form) {
        ...
        alert("Impossible de trouver un formulaire valide. Signaler cette erreur à Florian.");
    }
...
function GetId (monurl, mydiv, iddossier, id)
{
	mesparametres = 'iddossier='+iddossier+"&id="+id;
    ...
    $.ajax({
        ...
        error: function (html) {
            alert("Une erreur est survenue dans la fonction 'GetId'");
        }
    });
}

function UpdateId(myurl, mydiv, iddossier, libelleid, elt)
{
	myselect = $('#'+elt);
	if(myselect)
	{
		//myselectId = myselect.options[myselect.selectedIndex].value;
        myselectId = myselect.val();
	
		params = "u=GDP2:MAJ_ID&page_ok=&page_err=&iddossier="+iddossier+"&id="+myselectId+"&libelleid="+libelleid
		...
		/*var myAjax = new Ajax.Updater(
			...
			myurl,
			{
				method: 'post',
				parameters: params,
				//OnSuccess: UpdateText(mydiv, myselect)
				onSuccess: function(transport) { UpdateText(mydiv, elt, transport);},
				onFailure: function(transport) { UpdateText(mydiv, elt, transport); }
//				,
...
//				  }
        ...
        $.ajax({
            ...
            {
                UpdateText($("#" + mydiv), elt, transport); 
            },
...
function UpdateText(divElt, elt, res)
{
	//alert("divElt = " + divElt + " ; elt = " + elt + " ; res = " + res);
	myselect = $('#'+elt);
	//alert(myselect);
	if(myselect)
	{
		if(res.headerJSON == 1) 
		{
			imgTxt = '&nbsp;<img src="/z/backoffice/poliris_gestprod_v2/images/button_ok.png" width="12px" height="12px"/>';
			//texte = myselect.options[myselect.selectedIndex].text;
            texte = $("#" + elt + " option:selected").text();
		}
		else {
			texte = $('#'+divElt).innerHTML;
			imgTxt = '&nbsp;<img src="/z/backoffice/poliris_gestprod_v2/images/button_not_ok.png" width="12px" height="12px"/>';
		}

		$('#'+divElt).innerHTML = texte + imgTxt;
	}
}

function getType(elt_type, id)
{
