...
function affiche_pays_langue()
{
	...
	var txtPageRank=document.getElementById('txtPageRank');
	
	if(lstZone.value==1 )//Si internationale est choisi
	{
		txtPageRank.style.display='block';
	...
	{
		txtPageRank.style.display='none';
...
function AjouterUpdateAnnuaire()
{
	...
	u.value='GDP2:ADDUPDATE_ANNUAIRE';
...
function modify_annuaire(id)
{
	...
	if(zoneAnnuaire.value=='Nationale')//Si Nationale est choisi
	{
		...
		divVille.style.display='none';
		divDepartement.style.display='none';
		txtPageRank.text=pageRankAnnuaire.value;
		txtObservation.text=observationAnnuaire.value;
	...
	if(zoneAnnuaire.value=='Régionale')//Si régionale est choisi
	{
		...
		divVille.style.display='none';
		divDepartement.style.display='none';
		txtPageRank.value=pageRankAnnuaire.value;
	...
	if(zoneAnnuaire.value=='Départementale')//Si régionale est choisi
	{
		...
		if(zoneAnnuaire.value=='Locale')//Si régionale est choisi
	{
		txtPageRank.style.display='block';
		txtObservation.style.display='block';
		lblPageRank.style.display='block';
...
function validerAffectationannuaire()
{
	...
	for (i = 1; i < lstAnnuaireAAffecter.options.length; i++)
	{
		//selOptions[selOptions.length] = lstAnnuaireAAffecter.options[i];
		if(tabIdAnnuaireSel=='')
		{
			tabIdAnnuaireSel=lstAnnuaireAAffecter.options[i].value;
		}
		else
		{
			tabIdAnnuaireSel=tabIdAnnuaireSel + ',' + lstAnnuaireAAffecter.options[i].value;
		}
	}
	u.value='GDP2:AFFECTER_ANNUAIRE';
